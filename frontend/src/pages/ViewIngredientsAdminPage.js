...
const ViewIngredientsAdminPage = () => {
  ...
  const fetchData = async ({
    ...
    try {
      ...
      const response = await fetchAllIngredients(token, page, itemsPerPage, {
        search,
        sortBy,
        sortOrder,
      });
      ...
      if (data.length > 0) {
        ...
        const nutritionFields = keys.filter(
          (key, idx) => idx >= 3 && key !== 'isCore' && key !== "created_by" && key !== "clerk_id"
        );
  ...
  return (
    <div className='p-4'>
          ...
          <Button
            ...
            onClick={() => {
              ...
              navigate(`/add-ingredient${query}`, {
                state: { originalIngredient: selectedRows[0] || undefined },
              });
            }}
          />
          <Button
      ...
      >
        <Column selectionMode='multiple' frozen headerStyle={{ width: '3rem' }} style={{ width: '3rem'}} />
        <Column field='Name' header='Name' frozen sortable style={{ minWidth: '200px' }} headerStyle={{ minWidth: '200px' }} />
        <Column field="created_by" header="Created By" frozen sortable style={{ minWidth: '150px' }} headerStyle={{ minWidth: '150px' }} />
...
export default ViewIngredientsAdminPage;
